...
<?php if ( $this->should_print_form() ) : ?>
    ...
    <div class="cuar-js-wizard-section">
		...
		if ( ! $is_owner_selectable && empty( $default_owners ) ) {
			$this->print_submit_disabled_button( __( 'Done', 'cuarco' ), 0 );
	...
	<?php if ( $is_owner_selectable ) { ?>
        ...
        <div class="cuar-js-wizard-section">
			<?php
			$this->print_owner_field( __( 'Owner', 'cuarco' ) );
			$this->print_submit_button( __( 'Update', 'cuarco' ), 1 );
			?>
        </div>
	<?php } ?>
        ...
        (function ($) {
            ...
            $(document).ready(function () {
                ...
                if ($.isFunction($.fn.steps)) {
                    $(".cuar-form.cuar-update-content-form.cuar-customer-update-private-page-form").steps({
